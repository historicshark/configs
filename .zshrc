...
function backup() { 
    cd
    tar cfL - Documents Music Pictures budget | pigz > backups/backup_$(date -I).tar.gz
    cd -
}
...
function count() {
    ...
    for exclude in "$@"; do
        excludes+=(-not -path "*/$exclude/*")
    done

    if [ ${#excludes[@]} -gt 0 ]; then
        echo "Number of files in $dir (excluding $*): $(find -L "$dir" -type f "${excludes[@]}" | wc -l)"
    else
        echo "Number of files in $dir: $(find -L "$dir" -type f | wc -l)"
    fi
}
